...
void eval(char *cmdline)
{
	...
	if (argv[0] == NULL)
		return;
	//check if the command line is built in command
	if (!builtin_cmd(argv))
	{
		//setup for the blocking
		safe_sigemptyset(&mask);
		safe_sigaddset(&mask, SIGCHLD);
		...
		//& for last input argument run the program background
		if (bg)
		{
...
int builtin_cmd(char **argv) 
{
	...
	else if (!strcmp(argv[0], "bg") || !(strcmp("fg", argv[0])))
	{
		do_bgfg(argv);
		return 1;
	}
...
wrapper functions that handle error
**************************************/
